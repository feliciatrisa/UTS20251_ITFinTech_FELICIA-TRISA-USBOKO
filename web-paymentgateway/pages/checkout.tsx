...
export default function CheckoutPage() {
  ...
  const canPay = rows.length > 0 && total > 0 && /\S+@\S+\.\S+/.test(email);
  ...
  async function pay() {
    ...
    try {
      ...
      // Tampilkan pesan jelas kalau request ke Xendit gagal
      if (!res.ok) {
        alert(
    ...
    <div style={{ maxWidth: 900, margin: "24px auto", padding: 16 }}>
      ...
      <table width="100%" cellPadding={8} style={{ borderCollapse: "collapse" }}>
        ...
        <tbody>
          ...
          {rows.length === 0 && (
            <tr>
              <td colSpan={4} style={{ textAlign: "center", color: "#666", padding: 16 }}>
                Keranjang kosong. Kembali ke halaman utama untuk memilih item.
              </td>
            </tr>
          )}
        </tbody>
      ...
      >
        ...
        <button
          ...
          title={!canPay ? "Isi email & pilih item dulu" : "Bayar sekarang"}
        >
          Confirm & Pay
        </button>
      </div>
